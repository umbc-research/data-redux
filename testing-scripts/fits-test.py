...
logging.basicConfig(
    filename="{}/redux_{}.log".format(
        params.outdir, datetime.datetime.now().strftime("%Y%m%dT%H%M%S")
    ),
    encoding="utf-8",
...
try:
    ...
    for lightFilter in tqdm(
        params.fitsFiles["light"].keys(), desc="Calibrating Light Frames"
    ):
        ...
        for lightGain in params.fitsFiles["light"][lightFilter].keys():
            ...
            for lightIntTime in params.fitsFiles["light"][lightFilter][lightGain]:
                ...
                params.logger.info(
                    f"\t\tLooking up flats taken in filter {lightFilter}"
                )
                ...
                flatGain = (
                    lightGain  # These should always be equal ... but we should check!
                )
                ...
                flatIntTime = list(
                    params.fitsFiles["flat"][lightFilter][flatGain].keys()
                )[0]
                ...
                params.logger.info(
                    f"\t\tGot flats for filter {lightFilter}\n\t\t\t {flats}"
                )
                ...
                masterDarkForFlat, masterDarkFlatMap = redux_functions.accumulate(
                    darksForFlats, "dark"
                )
                params.logger.info(
                    f"Percent of Bad Pixels in Darks for Flats: \t {100*( masterDarkFlatMap.size- np.count_nonzero( masterDarkFlatMap))/ masterDarkFlatMap.size}"
                )

                # DO NOT KEEP THIS!!!!!!!!!!!!!!!!!!!!!!!1
                masterDarkForFlat[masterDarkForFlat <= 0] = 1

                masterDarkForFlatFrame = Frame(
                    masterDarkForFlat,
                    type="master",
                    filter=darksForFlats[0].filter,
                    gain=darksForFlats[0].gain,
                    intTime=darksForFlats[0].intTime,
                    header=darksForFlats[0].header,
                    badMap=masterDarkFlatMap,
                )
                ...
                params.logger.info(
                    f"\t\t\tGenerated Master Dark for Flat Calibration\n\t\t\t\t {masterDarkForFlatFrame}"
                )
                ...
                ### ACCUMULATE Flats ###
                masterFlat, masterFlatMap = redux_functions.accumulate(
                    [f.data - masterDarkForFlatFrame.data for f in flats], "flat"
                )
                ...
                masterFlatFrame = Frame(
                    masterFlat,
                    type="master",
                    filter=flats[0].filter,
                    gain=flats[0].gain,
                    intTime=flats[0].intTime,
                    header=flats[0].header,
                    badMap=masterFlatMap,
                )

                params.logger.info(
                    f"\t\t\t Generated master flat\n\t\t\t\t {masterFlatFrame}"
                )
                ...
                params.logger.info(
                    f"\tWorking to generate Master Dark for light calibration"
                )
                ...
                masterDarkForLights, masterDarkLightMap = redux_functions.accumulate(
                    darksForLight, "dark"
                )
                masterDarkForLightsFrame = Frame(
                    masterDarkForLights,
                    type="master",
                    filter=darksForLight[0].filter,
                    gain=darksForLight[0].gain,
                    intTime=darksForLight[0].intTime,
                    header=darksForLight[0].header,
                    badMap=masterDarkLightMap,
                )
                params.logger.info(
                    f"\t\t\tSet master dark for lights to {masterDarkForLightsFrame}"
                )
                darksForLight.setMaster(masterDarkForLightsFrame)

                params.logger.info(
                    f"\t\tGenerated master dark for light frame calibration"
                )
                ...
                masterLight, masterLightMap = redux_functions.accumulate(
                    [
                        (l - masterDarkForLightsFrame) / (masterFlatFrame.data)
                        for l in lights
                    ],
                    "light",
                )
                ...
                masterLightFrame = Frame(
                    masterLight,
                    type="master",
                    filter=lights[0].filter,
                    gain=lights[0].gain,
                    intTime=lights[0].intTime,
                    header=lights[0].header,
                    badMap=masterBadPixelMap,
                )
    ...
    try:
        if (sourceList == None) or (len(sourceList) == 0):
            params.logger.info(
                f"No sources found matching the DAOStarFinger parameterization. Running for a smaller FWHM Value (10 px)"
            )
            starFind = DAOStarFinder(
                threshold=finalLight.median,
                fwhm=10.0,
            ...
            if (sourceList == None) or (len(sourceList) == 0):
                params.logger.info(
                    f"No sources found matching the DAOStarFinger parameterization. Running for a smaller FWHM Value (5 px)"
                )
                starFind = DAOStarFinder(
                    threshold=finalLight.median,
                    fwhm=5.0,
                ...
                if (sourceList == None) or (len(sourceList) == 0):
                    params.logger.info(
                        f"No sources found matching the DAOStarFinger parameterization. Running for a larger FWHM Value (25 px)"
                    )
                    starFind = DAOStarFinder(
                        threshold=finalLight.median,
                        fwhm=25.0,
                    ...
                    if (sourceList == None) or (len(sourceList) == 0):
                        params.logger.info(
                            f"No sources found matching the DAOStarFinger parameterization. Running for a larger FWHM Value (35 px)"
                        )
                        starFind = DAOStarFinder(
                            threshold=finalLight.median,
                            fwhm=35.0,
                        ...
                        if (sourceList == None) or (len(sourceList) == 0):
                            params.logger.info(
                                f"No sources found matching the DAOStarFinger parameterization. Running for a larger FWHM Value (50 px)"
                            )
                            starFind = DAOStarFinder(
                                threshold=finalLight.median,
                                fwhm=50.0,
                            ...
                            if (sourceList == None) or (len(sourceList) == 0):
                                params.logger.info(
                                    f"No sources found matching the DAOStarFinger parameterization. YA DONE FUCKED UP"
                                )
                                sys.exit()
    except:
        pass
    ...
    plt.figure(1)
    plt.imshow(
        finalLight.data - finalLight.mean,
        cmap="gray_r",
        origin="upper",
        vmin=finalLight.mean - 2 * finalLight.std,
        vmax=finalLight.mean + 2 * finalLight.std,
    )

    for source in tqdm(
        sourceList, desc=f"Extracting Sources for Filter {finalLight.filter}"
    ):
        sourceID = source[0]
        xc, yc = source[2], source[1]
        loc = (xc, yc)
        subFrame = finalLight.data[
            int(xc - params.length) : int(xc + params.length),
            int(yc - params.length) : int(yc + params.length),
        ]
        subFramePixelMap = finalLight.badMap[
            int(xc - params.length) : int(xc + params.length),
            int(yc - params.length) : int(yc + params.length),
        ]
        radial_data_raw = redux_functions.extractRadialData(
            subFrame, xC=params.length, yC=params.length
        )[: params.length]
        ...
        backgroundCorrected2 = subFrame - background
        # DO NOT KEEP THIS!!!!!!!!!!!!!!!!!!!!!!!1
        backgroundCorrected[backgroundCorrected < 0] = 0
        backgroundCorrected2[backgroundCorrected2 < 0] = 0
        ...
        maskedSubFrame = np.ma.MaskedArray(backgroundCorrected2, mask=summationMask)
        counts = np.ma.sum(maskedSubFrame)

        print("counts:", counts, "no filter:", countsNoFilter)
        params.logger.info(
            f"Counts no pixelmap minus counts with pixel map:\t{countsNoFilter-counts}"
        )
        ...
        xLabels = np.concatenate(
            (
                np.linspace(0, params.length, 5)[::-1],
                np.linspace(0, params.length, 5)[1:],
            )
        ).astype(int)

        plt.xticks(
            np.linspace(0, params.length * 2, len(xLabels)), xLabels, rotation=45
        )
        ...
        plt.plot(
            pixelLocs, redux_functions.gaussian1D(pixelLocs, *fitparams[:-1], 0), "r"
        )
        plt.grid(1)
        ...
        plt.suptitle(
            f"Filter {finalLight.filter} PhotUtils Source ID {sourceID} w/o Background\nFit $R^2=${R2:0.4f}; $m={instMag:0.3f}$"
        )
        plt.savefig(
            f"{params.outdir}/subframe_{params.save}_{sourceID}_{finalLight.filter}.png"
        )
        ...
        plt.text(
            loc[1] + 5,
            loc[0] + 5,
            "{}, $m_{{inst}}=${:0.3f}".format(sourceID, instMag),
            color="k",
        )
    plt.figure(1)
    plt.savefig(
        "{}/finder_{}_{}_{}_{}_{}.png".format(
            params.outdir,
            params.save,
            finalLight.type,
            finalLight.filter,
            f"{int(finalLight.gain):d}",
            f"{finalLight.intTime:0.1f}".replace(".", "-"),
        )
    )
    plt.close(1)
